import { switch_theme_mode } from "./theme.mjs";
import { langdata } from "./langdata.mjs";

let currentArticleListPageOrder = 1; //这个变量表示目前在显示文章列表中的第几页，是从1开始数的

document.querySelector("#switch_theme_mode").onclick = switch_theme_mode;

const detect_whether_ui_load_finished = () => {
...
const copy_full_article_to_clipboard = () => {
  navigator.clipboard.writeText(document.querySelector("#article-content-html").innerText);
  document.querySelector("#copy_to_clipboard_button").innerHTML = `<i class="fa fa-check"></i> 已复制！`;
};

const preview_env_public_comment_fix = () => {
  if (window.location.href.indexOf("localhost") !== -1 || window.location.href.indexOf("127.0.0.1") !== -1) {
    if (blog["全局评论设置"]["valine设置"]["是否使用bbg公共评论服务"]) {
      document.querySelector("#vcomments").innerHTML = `<p style="color:red;">${langdata["COMMENT_UNAVAILABLE_DUE_TO_PREVIEW"][lang_name]}</p>`;
    };
  };
};
function resetPage() {
...
let bootstrap_css_import_finished = false;
let content_load_finished = false;
...
axios
  ...
  .then(function (response) {
    ...
    const lang_name = blog["网站语言"]; // TODO: need to fix!
